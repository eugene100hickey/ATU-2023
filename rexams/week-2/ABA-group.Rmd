...
Question
========
Run the code line below to read in a data frame called `ABAData`. 

```{r eval=FALSE}
brain_gene_expression <- read_rds("https://github.com/eugene100hickey/ATU-2023/blob/main/rexams/data/ABAData?raw=true") |> 
  filter(hgnc_symbol %in% glue::glue("c(\"{genes[1]}\", \"{genes[2]}\")"))
```

Use `group_by()` and `summarise()` to calculate the mean gene expression for each gene for each age category. Use thisto fill out the table below.
...
answerlist(c(glue::glue("{gene1_numbers}"), glue::glue("{gene2_numbers}")), markup = "markdown")
```

...
- we rarely know _population_ statistics about our data

- more common to have to infer them from _sample_ statistics
...
- if we don't know the real standard deviation
    ...
    - _t-distribution_ is more spread out (more uncertain) than the _normal_
    
    - as sample size gets bigger the _t_ converges to the _normal_
...
1. Make a 95% confidence interval for animal weight for the bisons recorded in the `knz_bison` dataset.

1. Perform a hypothesis test that female and  male bisons are the same weight.
